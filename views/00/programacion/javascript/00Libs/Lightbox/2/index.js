...
function lightbox_update_style(obj) {
  style.innerHTML = `
             #lightbox2-modal {
                  display: none;
                  background-color: ${obj.background ?? "rgba(255, 255, 255, 0.5)"
    };
                  position: fixed;
                  top: 0;
                  left: 0;
             ...
             #lightbox2-current-image {
                  ...
                  object-fit: contain;
                  border: 3px solid ${obj.colorBorder ?? "rgba(255, 255, 255, 1)"
    };
...
function lightbox_load() {
  ...
  lightbox2_imagenes = [...document.querySelectorAll("img")].filter(img=>img.dataset.lightbox || img.dataset.lightboxGroup);
  lightbox2_grupos = [];
  lightbox2_grupo = "0";
  lightbox2_currentImage = undefined;
  ...
  for (let i = 0; i < lightbox2_imagenes.length; i++) {
    ...
    if (!img.dataset.lightbox) {
      img.dataset.lightbox = img.dataset.lightbox;
    }
    ...
    document.getElementById("lightbox2-current-image").src = img.dataset.lightbox;
  }
